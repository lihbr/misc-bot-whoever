...
// src/whoever/index.ts
__export(exports, {
  handler: () => handler
});

// src/whoever/slack.ts
...
var verify = (event) => {
  if (!event.headers["x-slack-request-timestamp"] || !event.headers["x-slack-signature"]) {
    return false;
  }
  const ver = "v0";
  const timestamp = parseInt(event.headers["x-slack-request-timestamp"]);
  const body = event.body;
  if (process.env.NODE_ENV === "production" && Date.now() / 1e3 - timestamp > 60 * 5) {
    return false;
  }
  const basestring = `${ver}:${timestamp}:${body}`;
  const hmac = import_node_crypto.default.createHmac("sha256", process.env.SLACK_SIGNING_SECRET);
  hmac.update(basestring);
  const signature = `${ver}=${hmac.digest("hex")}`;
  return signature === event.headers["x-slack-signature"];
};
var get = async (endpoint, body) => {
  const res = await fetch(`${process.env.SLACK_API}${endpoint}?${import_qs.default.stringify(body)}`, {
    headers: {
...
var parseBody = (event) => {
  ...
  if (event.headers["content-type"] === "application/json") {
    ...
    const body = import_qs.default.parse(event.body);
...
var handler = async (event, _context) => {
  ...
  if (body.event.type === "app_mention") {
    ...
    if (body.event.thread_ts) {
      members = (await get("/conversations.replies", { channel: body.event.channel, ts: body.event.thread_ts, limit: 1e3 })).messages.map((m2) => m2.user);
    } else {
      members = (await get("/conversations.members", { channel: body.event.channel, limit: 100 })).members;
    }
    if (process.env.SLACK_BOT_IDS) {
      const ids = process.env.SLACK_BOT_IDS.split(",");
      members = members.filter((id) => !ids.includes(id));
    }
    const message = pickRandom(messages).replace("@user", `<@${pickRandom(members)}>`);
